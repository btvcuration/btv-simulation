...
export default function App() {
  ...
  const onDragEnd = (e) => { 
      ...
      if (dragType.current === 'BLOCK' && dragItem.current !== null && dragOverItem.current !== null) { 
          const _blocks = [...blocks]; 
          const dragBlock = _blocks[dragItem.current]; 
          _blocks.splice(dragItem.current, 1); 
          _blocks.splice(dragOverItem.current, 0, dragBlock); 
          setBlocks(_blocks); 
      } 
  ...
  const onDropFromInbox = (e, dropIndex) => { 
      ...
      if (str) { 
          ...
          if (req.type === 'BIG_BANNER') { 
              newBlock.type = 'BIG_BANNER'; 
              newBlock.banners = [{ title: req.title, desc: req.desc, landingType: 'NONE' }]; 
          } else if (['BAND_BANNER', 'LONG_BANNER', 'BANNER_1', 'BANNER_2', 'BANNER_3', 'MENU_BLOCK'].includes(req.type)) { 
              newBlock.type = req.type; 
              const bannerType = req.type === 'BANNER_1' ? '1-COL' : req.type === 'BANNER_2' ? '2-COL' : req.type === 'BANNER_3' ? '3-COL' : req.type === 'MENU_BLOCK' ? 'MENU' : undefined; 
              newBlock.banners = [{ title: req.title, type: bannerType, landingType: 'NONE' }]; 
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <select className="w-full bg-[#100d1d] border border-[#2e3038] rounded px-3 py-2 text-sm text-white focus:border-[#7387ff] outline-none" value={newRequestData.gnb} onChange={e => setNewRequestData({...newRequestData, gnb: e.target.value})}>
                            {gnbList.map(m => (<React.Fragment key={m.id}><option value={m.name}>{m.name}</option></React.Fragment>))}
                        </select>
                      </div>
                      <div>
